...
│   └── platform/           # Shared platform code (db, http, logger, storage, etc.)
├── sqlc/
...
2. **Define Entities**: Model your domain types and service interfaces inside each package.
3. **Write SQL Queries**: Update `sqlc/queries/` and your migration files for the target schema, then run `make sqlc`.
4. **Implement Repository**: Implement the repository layer using the generated `sqlc` package.
