...
- 🏗️ **Package-by-Feature Architecture** — Clean, modular domain organization (`internal/<your-domain>`) with high cohesion and low coupling.
- 🧩 **Go-Idiomatic Boundaries** — Consumer packages define interfaces, while `internal/platform/` stays focused on concrete implementations only.
...
```bash
# Update module path in go.mod to match your repository
go mod edit -module github.com/your-org/your-repo

# Update all Go import paths from github.com/username/go-api-kit
# to your own module path using your editor's global replace.
```
...
├── internal/
│   ├── auth/               # Authentication handlers and auth-domain interfaces
│   ├── docs/               # Swagger / docs handler
│   ├── health/             # Health check endpoints
│   └── platform/           # Concrete adapters only (db, http, logger, storage, etc.)
├── sqlc/
...
1. **Create Your Domains**: Add packages under `internal/` for your business areas, such as `users`, `orders`, or `billing`.
2. **Define Interfaces in the Consumer**: Put repository or provider interfaces in the package that uses them, such as a domain service package.
3. **Keep Platform Code Concrete**: Place third-party client implementations under `internal/platform/` and avoid defining interfaces there.
4. **Write SQL Queries**: Update `sqlc/queries/` and your migration files for the target schema, then run `make sqlc`.
5. **Wire Dependencies**: Connect repositories, services, handlers, and routes in `cmd/api/main.go`.
